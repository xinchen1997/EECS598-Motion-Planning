...
public:
    ...
    bool RRT(std::ostream& sout, std::istream& sinput)
    {
        ...
        NodeTree* Tree_init = new NodeTree(root, goal);
        NodeTree* Tree_goal = new NodeTree(goal, root);

        for(int i = 0; i < Tree_init->maxIter; i++)
        {
            cout << endl;
            cout << "iter: " << i << endl;

            // Tree_goal extend using RRTConnect
            RRTNode* rdNode = Tree_goal->probSample();
            RRTNode* nearestNode = Tree_goal->findNearestNode(rdNode);
            RRTNode* targetNode;

            cout << "distance: " << Tree_goal->distance(nearestNode->getConfig(), rdNode->getConfig()) << endl;
            if (Tree_goal->distance(nearestNode->getConfig(), rdNode->getConfig()) < 1e-1)
            {
                targetNode = rdNode;
            }
            else{
                targetNode = RRTConnect(Tree_goal, rdNode, nearestNode);
            }

            if (!collisionCheck(targetNode->getConfig())) {
                Tree_goal->addNode(nearestNode, targetNode);
                // cout << "parentNode: ";
                // targetNode->printParentInfo();
            }

            Tree_init->_goal = targetNode;
            RRTNode* rdNode_2 = Tree_init->probSample();
            RRTNode* nearestNode_2 = Tree_init->findNearestNode(rdNode_2);
            RRTNode* newNode;

            cout << "distance: " << Tree_init->distance(nearestNode_2->getConfig(), rdNode_2->getConfig()) << endl;
            if (Tree_init->distance(nearestNode_2->getConfig(), rdNode_2->getConfig()) < 1e-1)
            {
                newNode = rdNode_2;
            }
            else{
                newNode = RRTConnect(Tree_init, rdNode_2, nearestNode_2);
            }

            if (!collisionCheck(newNode->getConfig())) {
                Tree_init->addNode(nearestNode_2, newNode);
                // cout << "parentNode: ";
                // newNode->printParentInfo();
            }

            Tree_goal->_goal = newNode;
            ...
            // cout << "targetNode: ";
            // targetNode->printNodeInfo();

            if (Tree_init->isGoal(newNode)) {
                cout << "reach goal" << endl;
                end = clock();
                Tree_init->getPath(newNode);  // path from init to newNode
                // Tree_init->printPath();
                Tree_goal->getPath(targetNode);  // path from goal to target
                // Tree_goal->printPath();

                // combine two paths
                NodeTree* Tree_bidirection = new NodeTree(root, goal);
                reverse(Tree_goal->_path.begin(), Tree_goal->_path.end());
                // Tree_goal->printPath();
                Tree_bidirection->_path.insert(Tree_bidirection->_path.end(), Tree_init->_path.begin(), Tree_init->_path.end());
                Tree_bidirection->_path.insert(Tree_bidirection->_path.end(), Tree_goal->_path.begin(), Tree_goal->_path.end());
                // Tree_bidirection->printPath();
                plotRed(Tree_bidirection->_path);

                double unsmoothed_path_length = getPathLength(Tree_bidirection->_path);

                cout << "smoothing path" << endl;
                shortcutSmoothing(Tree_bidirection);

                cout << "draw trajectory" << endl;
                moveTraj(Tree_bidirection->_path);
                plotBlue(Tree_bidirection->_path);

                cout << "goal bias: " << Tree_init->probablity << endl;
                cout << "Run time: " << (double)(end - start) / CLOCKS_PER_SEC << " seconds" << endl;
                cout << "Number of nodes sampled:" << Tree_init->sample_nodes.size() + Tree_goal->sample_nodes.size() << endl;
    ...
    double getPathLength(std::vector<vector<dReal>> path)
    {
        double length = 0;
    ...
    double dis(vector<dReal> q1, vector<dReal> q2)
    {
        double dis = 0.0;
        for(int i = 0; i < DOF; i++)
        {
