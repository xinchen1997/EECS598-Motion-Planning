...
if __name__ == "__main__":
    ...
    # env.Load('pr2test2_difficult.env.xml')
    time.sleep(0.1)
    ...
    #### Implement your algorithm to compute a path for the robot's base starting from the current configuration of the robot and ending at goalconfig. The robot's base DOF have already been set as active. It may be easier to implement this as a function in a separate file and call it here.
        planning = anastar.ANAstar(goalconfig)
        planning.eightConnectedANAstar(env, robot)
    ...
    lastconfig = planning.path[0]
    for i in range(1, len(planning.path)):
        currentconfig = planning.path[i]
        handles.append(env.drawlinestrip(points=array(((lastconfig[0], lastconfig[1], 0.3), (currentconfig[0], currentconfig[1], 0.3))),
                                         linewidth=5.0,
                                         colors=array(((0,0,0)))))
